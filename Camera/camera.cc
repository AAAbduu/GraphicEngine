...
int Camera::checkFrustum(const BBox *theBBox,
	...
	/* =================== PUT YOUR CODE HERE ====================== */
	//Como tenemos el archivo intersection.h, podemos usar las funciones que hay en el
	//Utilizaremos la funcion BBOXPLANEINTERSECT
	int cont = 0; //Contador para saber si el bbox esta completamente dentro del frustum
	for(int i = 0; i < MAX_CLIP_PLANES; i++){
		int intersecta = BBoxPlaneIntersect(theBBox, m_fPlanes[i]); //comprobamos con el plano i-esimo
		if(intersecta == 1) { //EL bbox esta completamente fuera del plano
			return 1;
		}
		else if(intersecta == -1){ //El bbox esta en el lado positivo del plano, no sabemos nada hasta que 
								//comprobemos con el resto de planos
			cont++;
		}
	}
	
	if(cont == MAX_CLIP_PLANES) res = -1; //El bbox esta completamente dentro del frustum
	else res = 0; //El bbox esta parcialmente dentro del frustum, si no se ha rechazado que está completamente fuera o dentro, entonces, intersecta.
